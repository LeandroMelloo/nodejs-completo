...
function  formatValue(value){
    let str = parseFloat(value).toFixed(2).concat("");
    str = str.replace(".",",");
    str = "$ ".concat(str);
    return str;
}
...
function deleteData(id) {
    if(confirm("Delete this item?")) {
        if(id === list.length - 1) {
            ...
            console.log(arrAuxIni) // pega o primeiro registro
            let arrAuxEnd = list.slice(id + 1);
            console.log(arrAuxEnd) // pega o ultimo registro
...
function validation() {
    ...
    if(desc === "") {
        ...
        errors += '<p>Registro de quantidade invalida</p>'
    }
    if(value === "") {
        errors += '<p>Digite um valor</p>'
    } else if(value != parseFloat(value)) {
        errors += '<p>Registro de valor invalido</p>'
    }
    ...
    if(errors != "") {
        ...
        document.getElementById("errors").innerHTML = "<h3>Error:</h3>".concat(errors);
        return 0;
    } else {
        return 1;
    }
